...
class CryptoTradingEnvironment:
    def __init__(self, data, initial_capital=1000, n_days=5, sharpe_window=30, slippage_factor=0.005, stop_loss_pct=0.1): # Added sharpe_window, slippage_factor, and stop_loss_pct
    ...
    def reset(self):
        self.current_step = 0 # random.randint(0, 6000) # TODO - Set to random
    ...
    def step(self, actions):
        ...
        for i, coin in enumerate(self.coins):
            ...
            if self.holdings[coin] > 0 and self.peak_prices[coin] is not None: # Only check stop loss if we hold the coin and have recorded a peak price
                if next_price < self.peak_prices[coin] * (1 - action_set[f'{coin}_sl']):
...
class TD3Agent:
    def __init__(self, state_dim, hist_dim, action_dim):
        ...
        self.optimizer_actor = tf.keras.optimizers.Adam(learning_rate=1e-5)
        ...
        self.optimizer_critic_1 = tf.keras.optimizers.Adam(learning_rate=1e-5)
        self.optimizer_critic_2 = tf.keras.optimizers.Adam(learning_rate=1e-5)
    ...
    def _build_actor_model(self):
        ...
        # )
        # x = positional_embedding_layer(hist_norm)
        ...
        # )(x, x)
        # x = tf.keras.layers.Add()([hist_norm, attn_output])
        ...
        # dense0 = tf.keras.layers.Dense(32, activation='relu')(flattened)
        ...
        dense0 = tf.keras.layers.Dense(1024, activation='relu')(hist_flattened) 
        ...
        dense2 = tf.keras.layers.Dense(512, activation='relu')(dense1) # Reduced size
        stop_losses = tf.keras.layers.Dense(math.floor(self.action_dim / 2), activation='sigmoid')(dense2)
        split_pcts = tf.keras.layers.Dense(math.floor(self.action_dim / 2) + 1, activation='softmax')(dense2)
        output = tf.keras.layers.Concatenate()([split_pcts, stop_losses])
...
if __name__ == '__main__':
    ...
    timesteps_per_episode = 50  # Length of each episode
    batch_size = 64  # Batch size for training updates
    buffer_capacity = 100000
    train_actor_every_step = 2 # Delayed actor updates
    sharpe_window = 3000000 # Window size for Sharpe Ratio calculation
    ...
    env = CryptoTradingEnvironment(train_data, n_days=14, sharpe_window=sharpe_window, slippage_factor=slippage_factor, stop_loss_pct=stop_loss_pct)
    current_state_dim = (9,)
    ...
    for episode in range(episodes):
        ...
        plt.title(f'Episode {episode+1} Portfolio Value Comparison)') # Updated chart title
        plt.legend()
